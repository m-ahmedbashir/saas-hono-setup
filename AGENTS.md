...
- No custom `roles`/`ac` passed to the plugin — it uses its own built-in default permission statements (`user`: create/list/set-role/ban/impersonate/impersonate-admins/delete/set-password/set-email/get/update; `session`: list/revoke/delete). Nothing in this app needs finer-grained platform-admin tiers yet; add a custom `roles`/`ac` here specifically (not `packages/core/src/auth/permissions.ts`'s) if that ever changes.
- `admin-plugin.integration.test.ts` (`apps/api/src/modules/auth/`) proves this is actually wired, not just configured: a regular user gets 403 from `/api/auth/admin/list-users`; a `user.role: "admin"` account (created via the trusted server-side `createUser` path above, since `ADMIN_USER_IDS` is read once from `process.env` at module load and can't include a not-yet-generated test user's id) can list users and ban another user, whose existing session is then immediately dead.
...
- **Form validation is Zod, UX-only — the server stays the actual authority.** E.g. `src/features/auth/schemas/sign-in.schema.ts` checks email format and non-empty password to fail fast before a network round-trip; it deliberately doesn't duplicate Better Auth's exact password-length rules. Forms use TanStack Form via `useAppForm`/`useFormFields` (`src/components/ui/tanstack-form.tsx`) — see the `kiranism-shadcn-dashboard` skill for the full form/table/query conventions this template already establishes; follow those for anything not auth-specific.
