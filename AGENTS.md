...
- **A route's real logic never lives in `.routes.ts` — this is the hard rule, not a style preference.** `.routes.ts` is a manifest: reading it should tell you every endpoint, its method, which guards protect it, and what it calls — not how any of that actually works. Concretely: a route registration lists middleware (named, or a validator bound inline to that one route — see below), then a handler body that reads input, calls *one* thing, and shapes the response. The moment a handler needs multiple sequential steps — a `switch`/`if` chain deciding what an input *means* (routing one event type to one DB update, another to a different one), or several calls happening in order — that's real logic, and it belongs in a separate file the route imports: `<feature>.service.ts` for logic a route calls directly, `<feature>.handlers.ts` for logic reacting to something else (an event, a webhook). Same concept, name it for what triggers it. `billing.routes.ts`'s webhook route is the reference example: verify-signature + parse + conditional-dispatch used to be three inline steps; now it's one call to `billing.handlers.ts`'s `processWebhook` (which internally calls the private `handleBillingEvent` for the event-type `switch`).
  - **Middleware/validators are a judgment call, not mandatory extraction.** Reusable across routes → a named middleware in `apps/api/src/middleware/` (`injectUserContext`, `requirePermission`, `requireAllowedOrigin`, `requireSelfParam`). One-off wiring for a single route (e.g. a `zValidator` bound to that route's own schema) → fine to define inline in `.routes.ts`, like `billing.routes.ts`'s `validateCheckoutBody`. Neither is a violation; extract only when something would actually be reused.
  - **The one standing exception to "call one thing": narrowing `UserContext`.** Hono's context typing doesn't narrow the `B2C | B2B2C` union across a middleware/handler boundary, so a handler needing `organizationId` still needs one call to narrow it even after `injectUserContext` has already run at the type level. `apps/api/src/middleware/auth.middleware.ts`'s `requireOrgContext(userContext, message)` is that one call — it's narrowing glue, not business logic, so it doesn't count against the "one call" rule the way a second *service* call would. `billing.routes.ts`'s `/checkout` handler is the reference example.
  - Most routes (e.g. `/billing/checkout`) are genuinely just middleware → a validator → a handler that calls one service function and responds. That's already correct — don't add a `.handlers.ts`/`.service.ts` a route doesn't need, and don't extract a guard nothing else will ever reuse.
...
- Every route that accepts a body/query validates it with `@hono/zod-validator`'s `zValidator(target, schema, hook)` applied as route middleware — a genuine pre-route guard, not a `.safeParse()` call as the first lines of the handler. The handler then reads `c.req.valid("json")` (or `"query"`/etc), never `c.req.json()` directly. The `hook` is required, not optional: without it, a validation failure returns `@hono/zod-validator`'s own response shape, which doesn't match this app's `{ success, error }` envelope — the hook must re-throw via `AppError("VALIDATION_ERROR", message, details)` so every failure still goes through the one centralized path. `apps/api/src/modules/billing/billing.routes.ts`'s `validateCheckoutBody` is the reference example. Building the `details` payload for a Zod v4 failure needs `flattenError(result.error)` imported from `"zod"` — `result.error` inside the hook is zod's core `$ZodError`, which doesn't have a `.flatten()` method the way a `.safeParse()` result's classic `ZodError` does.
