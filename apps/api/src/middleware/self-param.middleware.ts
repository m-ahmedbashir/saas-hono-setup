...
export const requireSelfParam = (paramName: string, message = `Cannot act on another user's ${paramName}`) =>
  createMiddleware(async (c, next) => {
    const userContext = c.get("userContext");
    if (!userContext) {
      throw new AppError("INTERNAL_ERROR", "requireSelfParam used without injectUserContext running first");
    }
